...
(function (l) {
	"function" === typeof define && define.amd
		...
		? (module.exports = function (z, A) {
				...
				A ||
					(A =
						"undefined" !== typeof window ? require("jquery") : require("jquery")(z));
	...
	function Za(a) {
		...
		if (d || (!t && !h && !e && g == oa(a) && g == m.length))
			...
			for (v = 0; v < k.length; v++)
				(w = c[k[v]]),
					(m[v].style.width =
						null !== w.sWidthOrig && "" !== w.sWidthOrig ? K(w.sWidthOrig) : ""),
					w.sWidthOrig &&
						h &&
						l(m[v]).append(
							l("<div/>").css({
	...
	B.extend = function (a, b, c) {
		if (c.length && b && (b instanceof B || b.__dt_wrapper)) {
			...
			for (d = c.length; h < d; h++) {
				var f = c[h];
				b[f.name] =
					"function" === f.type ? e(a, f.val, f) : "object" === f.type ? {} : f.val;
	...
	B.register = y = function (a, b) {
		if (Array.isArray(a))
			...
			for (c = d.length; a < c; a++) {
				...
				g ||
					((g = { name: f, val: {}, methodExt: [], propExt: [], type: "object" }),
					e.push(g));
				a === c - 1
					? ((g.val = b),
					  (g.type =
	...
	var ub = function (a, b, c, d, e) {
			...
			for (g = b.length; m < g; m++) {
				var n =
					b[m] && b[m].split && !b[m].match(/[\[\(:]/) ? b[m].split(",") : [b[m]];
				var p = 0;
					...
					for (p = e.length; n < p; n++)
						for (g = e[n], t = 0; t < k; t++)
							(v = { row: g, column: t }),
								r
									? ((x = d[g]),
									  w(v, T(a, g, t), x.anCells ? x.anCells[t] : null) && m.push(v))
									: m.push(v);
					return m;
				}
	...
	u.defaults = {
		...
		oLanguage: {
			...
			sInfoFiltered: "(กรองจาก จาก _MAX_ รายการทั้งหมด)",
			sInfoPostFix: "",
			sDecimal: "",
			sThousands: ",",
			sLengthMenu: "",
